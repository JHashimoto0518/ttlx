...
func TestBuild_SimpleConfig(t *testing.T) {
	...
	// 3. TTL生成
	ttls, err := generator.GenerateAll(cfg, "simple.yml")
	require.NoError(t, err)
...
func TestBuild_FullConfig(t *testing.T) {
	...
	// 3. TTL生成
	ttls, err := generator.GenerateAll(cfg, "full.yml")
	require.NoError(t, err)
	require.NotEmpty(t, ttls)
	...
	assert.Contains(t, ttl, "expandenv connectcmd 'bastion.example.com:22")
