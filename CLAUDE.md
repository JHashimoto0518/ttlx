...
1. **機能ブランチの作成**
   ...
   # 例: feat/add-new-feature, fix/bug-123, docs/update-readme
   ```

2. **変更の実装とコミット**
   - 機能ブランチで作業
   - 適切なコミットメッセージで段階的にコミット
...
**例外: 直接mainにコミット可能なケース**
- ドキュメントの誤字修正など、極めて軽微な変更
- 緊急のhotfix（事後報告必須）

上記以外は**必ずPRを作成**してください。
...
すべての変更でステアリングドキュメントが必要なわけではありません。
以下の基準で判断してください。

##### ステアリングドキュメントが**必要**なケース
...
3. **非自明な修正**
   - 影響範囲が広いバグ修正
   - パフォーマンス改善のための設計変更
   - セキュリティ対応で設計レビューが必要な場合

4. **ユーザー確認が必要な場合**
   - 実装方針について意見を聞きたい
   - 仕様の解釈に迷いがある

##### ステアリングドキュメントが**不要**なケース
...
**迷ったら**: ユーザーに確認するか、ステアリングドキュメントを作成する方向で判断してください。
...
1. **影響範囲の特定**
   ...
   ```bash
   # コード例を含むドキュメントを検索
   grep -l "<検索パターン>" docs/*.md README*.md **/README*.md
   ```

2. **更新対象のリストアップ**
   - 更新が必要なドキュメントと箇所を一覧化
   - 更新不要と判断したものも理由とともに提示
   - ユーザーの承認を得てから更新を開始
...
**重要**: ベータ版の後方互換性ポリシーは、プロジェクト開始時に必ずユーザーに確認してください。

**確認事項:**
- ベータ版でも後方互換性を担保するか？
- 破壊的変更時に詳細な移行ガイドを作成するか？
- CHANGELOGに破壊的変更を詳細に記載するか？

**判断基準:**
- **ユーザー数**: ベータ版でも多くのユーザーが見込まれる場合は、後方互換性を重視
- **仕様の安定性**: 仕様が頻繁に変わる場合は、設計のシンプルさを優先
- **プロジェクトの性質**: エンタープライズ向けは互換性重視、個人ツールは柔軟に
